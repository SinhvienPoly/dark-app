...
import { ExtandVideo } from '@/types';

type Props = {
    categories: Category[];
    data?: ExtandVideo | null;
};

const VideoUpdateForm = ({ categories, data }: Props) => {
    const router = useRouter();
    const [isPending, startTransition] = useTransition();

    const [tags, setTags] = useState<string[]>(data?.tags!);
    const [categoryIds, setCategoryIds] = useState<string[]>(
        data?.category_for_videos?.map((item) => item.category_id!)!,
    );
    const [thumbnail, setThumbnail] = useState<ImageType | null>(data?.thumbnail!);
    const [video, setVideo] = useState<ImageType | null>(data?.video!);
    ...
    const isChecked = (id: string) => {
        return categoryIds?.some((item) => item === id);
    };
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input defaultValue={tags?.join(',')!} placeholder="tag...." onChange={(e) => onTags(e)} />
